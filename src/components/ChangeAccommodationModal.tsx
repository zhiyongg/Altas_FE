...
import { stayOptionsList } from '../data/mockTripData';
...
export const ChangeAccommodationModal: React.FC<ChangeAccommodationModalProps> = ({
  ...
  const filteredStays = stayOptionsList.filter((stay) => {
    if (searchQuery.trim()) {
      const q = searchQuery.toLowerCase();
      return stay.name.toLowerCase().includes(q) || stay.location.toLowerCase().includes(q);
    }
    return true;
  }).sort((a, b) => {
  ...
  const featuredStays = filteredStays.filter((s) => s.isSponsored);
  const regularStays = filteredStays.filter((s) => !s.isSponsored);
                    ...
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <div className="flex items-center gap-1.5 bg-[#f3f4f5] w-fit px-2.5 py-1 rounded-md">
                          ...
                          <span className="text-xs font-bold text-[#191c1d]">{stay.rating}</span>
                          <span className="text-xs text-[#727785]">({stay.reviewsCount.toLocaleString()} reviews)</span>
                        </div>
                      </div>

                      <button
                        onClick={(e) => toggleFav(stay.id, e)}
                        className="p-2 rounded-full hover:bg-[#f3f4f5] transition-colors text-[#727785] cursor-pointer"
                      >
                        <span className={`material-symbols-outlined text-[20px] ${favorites[stay.id] ? 'fill text-[#ba1a1a]' : ''}`}>
                          favorite
                        </span>
                      </button>
                    </div>
